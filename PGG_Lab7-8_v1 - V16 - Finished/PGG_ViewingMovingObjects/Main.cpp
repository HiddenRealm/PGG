...
int main(int argc, char *argv[])
{
	...
	int winWidth = 640;
	int winHeight = 640;
	SDL_Window *window = SDL_CreateWindow("PGG - Assignment 1",  // The first parameter is the window title
		winPosX, winPosY,
		winWidth, winHeight,
	...
	while (go)
	{
		...
		while (SDL_PollEvent(&incomingEvent))
		{
			...
			myScene.Movement(incomingEvent, deltaTs);
		...
		//Update the scene - its bool so i can shut the while loop
		go = myScene.Update(deltaTs);
