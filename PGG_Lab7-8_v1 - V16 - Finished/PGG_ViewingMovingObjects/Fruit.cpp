...
{
	srand(time(NULL));
	//picks the three spawns
	Rand();

	//sets the first one in place
	_pos = _ran1;
...
bool Fruit::Update(int value)
{
	...
	if (_collected == 1)
	{
		...
		//plays this sound effect (im aware PlaySound is not a good way to implement audio, i was bound by time)
